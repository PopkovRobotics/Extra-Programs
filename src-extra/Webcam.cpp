 ...
 */
Webcam::Webcam(const std::string& device, int width, int height) :
                        device(device),
                        xres(width),
                        yres(height) {
    // Открываем вебкамеру
    ...
    frame_array = new int** [xres];

    for(size_t x = 0; x < xres; x++) {
        frame_array[x] = new int* [yres];
        for(size_t y = 0; y < yres; y++) frame_array[x][y] = new int[3];
    }
...
Webcam::~Webcam() {
      ...
      for(size_t x = 0; x < xres; x++) {
          for(size_t y = 0; y < yres; y++) {
              delete[] frame_array[x][y];
          }
          delete[] frame_array[x];
      }
      delete[] frame_array;
}
...
int*** Webcam::frame(int timeout) {
    ...
    for (;;) {
        ...
        if (read_frame()) {
            for(size_t y = 0; y < yres; y++) {
                for(size_t x = 0; x < xres; x++) {
                    size_t start = (y * (xres * 3) + (x * 3));
                    for(size_t i = 0; i < 3; i++) frame_array[x][y][i] = data[start + i];
                }
            }
